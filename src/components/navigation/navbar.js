...
function Navbar() {
	...
	return (
		<nav
			id='navbar'
			className={`${isSticky ? 'sticky top-0' : 'absolute top-[3.438rem]'} z-10 w-full bg-transparent transition-all duration-500 ease-in-out`}
		>
						...
						<div className='flex gap-x-6'>
							{navMenuItems.map((item) => {
								...
								return (
									<div
										key={item.id}
										className={`relative flex cursor-pointer items-center justify-center text-center hover:text-secondary-300 dark:hover:text-secondary-200 ${isActive ? 'text-secondary-500' : 'text-textColor-500 dark:text-gray-300'} group`}
										onClick={() => changeRoute(router, item.slug)}
									>
										{item.name}
										<span className={`absolute bottom-[-2px] left-0 h-[2px] w-full scale-x-0 transform transition-transform duration-300 ease-in-out group-hover:scale-x-100 ${isActive ? 'scale-x-100 bg-secondary-500' : 'bg-secondary-300 dark:bg-secondary-200'}`}></span>
									</div>
								);
							})}
							<ToggleTheme />
